<template>
  <div class="form-box">
    <el-tag type="info">添加垃圾转运站信息</el-tag>
  ...
  methods: {
    async postAxios() {
      const { data: res } = await axios({
        method: "POST",
        data: {
          rtsName: this.form.name,
          rtsAddress: this.form.region + this.form.address,
          rtsArea: this.form.area,
          lng: this.form.lng,
          lat: this.form.lat
        },
        url: "http://localhost:8088/tanghao/api/insertRts"
      });
      return res;
    },
    onSubmit() {
