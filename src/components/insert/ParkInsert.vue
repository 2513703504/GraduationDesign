<template>
  <div class="form-box">
    <el-tag type="info">添加环卫停车场信息</el-tag>
  ...
  methods: {
    async postAxios() {
      const { data: res } = await axios({
        method: "POST",
        data: {
          svpName: this.form.name,
          svpAddress: this.form.region + this.form.address,
          svpArea: this.form.area,
          lng: this.form.lng,
          lat: this.form.lat
        },
        url: "http://localhost:8088/tanghao/api/insertSvp"
      });
      return res;
    },
    onSubmit() {
